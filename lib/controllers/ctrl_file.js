  ...
  sync.waterfall([
    ...
    function(done) {
      file.getInfo(code, fileInfoId, function(err, fileInfoData) {
        if (err) {
          log.error(err, uid);
          done(new errors.InternalServer(__("js.ctr.common.system.error")));
        } else if (_.isNull(fileInfoData)) {
          done(new errors.NotFound(__("js.ctr.common.system.notfound")));
        } else {
          result.fileInfo = fileInfoData;
          done(undefined, fileInfoData);
        }
      });
    },
