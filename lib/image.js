 ...
 * @param callback
 */
exports.cropAndThumb = function(handler, fileIndex, callback) {

  var srcFile = handler.params.files[fileIndex]
    , result = {};

  sync.forEach(handler.params.crops, function(item, doneCrop) {
    ...
    exports.crop(src, out, item.width, item.height, item.x, item.y, function(err) {
      ...
      // 对裁剪之后的图做成缩略图
      sync.forEach(handler.params.thumbs, function(item, doneThumb) {

        var thumbSrc = out
          , thumbOut = out + item.key;

        exports.thumbnail(thumbSrc, thumbOut, item.width, item.height, item.quality, function(err) {
          if (err) {
            doneThumb(err);
            return;
          }
          ...
          // 生成Handler，添加文件对象
          var newHandler = new context().create(handler.uid, handler.code, handler.lang);
          newHandler.addParams("files", [{ name: srcFile.name, path: thumbOut, type: srcFile.type }]);
